# Growth Algorithm — Global (Research-Backed)

> **Last updated:** Feb 11, 2026
> **Purpose:** Universal platform knowledge for X/Twitter growth. Persona-specific details (target accounts, timing, content calendar) belong in each persona's `_growth-algorithm.md`.

---

## X Algorithm Intelligence
...
- Engagement from HIGH-authority accounts (10K+ followers) is weighted dramatically more than from small accounts
- One like from a 500K+ account > 50 likes from random accounts
...
4. CROSS-DOMAIN replies (non-niche viral posts = bigger audience)
5. QUOTE TWEETS (your take reaches both audiences)

PRIORITY ORDER (Communities + Premium):
1. Reply to viral posts (5K+ likes ideal) — 40% of time
2. Original posts TO COMMUNITIES — 20% of time ← #1 LEVER for new accounts
3. Reply to trending/cultural posts — 15% of time
4. Quote tweet trending posts — 10% of time
5. News-reactive replies/posts — 10% of time
6. Threads (3-6 tweets) — 5% of time
```
...
**Why this matters:** Original posts from accounts under 1K followers get 8-9 views on the main feed — even with Premium. Posts to Communities bypass this cold-start problem entirely because they're shown to the community's full member base, not just your followers.

### Community Posting Rules

```
PHASE 1 (Under 3K followers):
├── Post 100% of original content TO communities (not main feed)
├── Communities give your content an instant audience of thousands
├── Your followers ALSO see community posts in their feed
├── Net effect: same reach to followers + massive community bonus
└── 2-3 community posts/day during money window

CONTENT FOR COMMUNITIES:
├── Build-in-public stories (what you shipped, what broke)
├── Technical takes relevant to the community's topic
├── Career/life content where it fits
├── Contrarian takes on trending topics
└── IMAGES get 2-3x boost in communities too

WHAT NOT TO POST TO COMMUNITIES:
├── Pure self-promotion ("check out my product")
├── Generic motivational content
├── Content that doesn't match the community's topic
└── Same post to multiple communities simultaneously (feels spammy)

PHASE 2 (3K-5K followers):
├── Split: 50% community posts, 50% main feed
├── Main feed posts now have enough followers for seed engagement
└── Communities still valuable for reaching new audiences

PHASE 3 (5K+ followers):
├── Main feed posts get organic distribution
├── Use communities strategically for high-value content
└── Communities become supplemental, not primary
```

### Engagement Velocity Bootstrapping

```
THE PROBLEM: First 30 minutes determine 80%+ of a post's reach.
New accounts have no one to engage in those first 30 minutes.

SOLUTIONS:

1. COMMUNITIES (primary)
   └── Community members see your post immediately → instant engagement
       This alone solves the velocity problem for new accounts

2. GROWTH COHORT (5-8 builders at your level)
   ├── Share your post in the DM group when you publish
   ├── They engage genuinely (not a pod — selective, not systematic)
   ├── You do the same for their content
   └── 2-3 real engagements in first 10 min = algorithm boost

3. CROSS-PLATFORM NOTIFICATION
   ├── Post on X → share link on other platforms (LinkedIn, etc.)
   ├── Your existing audience on other platforms can drive initial engagement
   └── Even 5-10 clicks = engagement signal for X algorithm

4. SELF-REPLY THREAD
   ├── Post your content → immediately reply with a question or TLDR
   ├── Your self-reply counts as a reply signal
   ├── Encourages others to reply too (lower barrier than first comment)
   └── "Curious what others are seeing — is this just us?"
```

---

## Reply Decision Tree (Value-First)
...
│   ├── Data/comparison? → Generate graphic
│   ├── Concept explanation? → Consider diagram or visual
│   └── Humor? → Only if image genuinely adds to the joke
└── Draft → EDITOR GATE → Post

KEY RULE: VALUE FIRST, EXPERIENCE SECOND.
"What does the reader learn?" > "What have I done?"
```

### Reply Success Patterns

| Pattern | % of successful replies | Description |
|---------|----------------------|-------------|
| "Actually" correction with data | 30% | Correct a popular post with specifics |
...
**Every piece of content MUST pass through this review loop before posting.**
...
⚠️ FORMULA CRUTCH:
Don't use the same reply structure repeatedly. Rotate approaches:
...
7. Experience Story: [ONLY when genuinely unique — MAX 30%]

⚠️ BANNED PATTERNS:
- "2am/3am" device — overused
- Starting with "Lived this" or "Built this" more than 1 in 5 replies
- "The thing most people miss is..." — overused opener
- Any reply that could work on 10+ similar posts → too generic
```

### Step 3: CONTENT TYPE EDITOR
...
├── OK: Add a non-US/underrepresented perspective
...
- [ ] Zero banned phrases (see `strategy/_core-principles.md`)
...
├── Data/comparison → YES: Generate graphic with the data
...
1. NEWS REACTION:
   # Extract article's featured image via og:image meta tag
   # ADD SOURCE CREDIT: Include article source in tweet AND on image

2. DATA/COMPARISON GRAPHIC:
   # Generate with Pillow (see skills/content/image-sourcing.md for templates)
   # Standard size: 1200x675 (16:9)
   # Always include @handle attribution on image
...
4. STOCK PHOTOS (rare — only for career/life posts):
   # Unsplash API or similar free stock photo service
```
...
└── Your own graphics: Include your @handle watermark
...
KEY: Bio must signal PERSONALITY, not just expertise.
Someone who liked your humor reply should see the bio
and think "this person is interesting" not "this person is serious."
```

### Pinned Tweet Strategy
- **Best option:** A career story/thread that's inherently interesting
- **Alternative:** Your best-performing tweet (social proof)
- **Alternative:** An expertise thread showing depth
...
├── Profile visits to follow: typical 5-15% (good profile)
├── Under 5%: bio/content mismatch, boring bio, no pinned tweet
├── Over 15%: strong brand alignment, compelling pinned tweet

CROSS-DOMAIN CONVERSION FRICTION:
Someone loves your humor reply on a sports post → clicks profile →
sees "[Serious Job Title]" bio → doesn't follow because content doesn't match.

FIXES:
1. Bio signals personality (not just expertise)
2. Pinned tweet = conversion anchor (best content first)
3. Recent tweets show range (never 3+ tweets on same topic in a row)
```
...
```
PREMIUM FEATURES:
├── Reply boost: 2-4x more visibility in reply threads
...
├── Edit typos within 1 hour (but avoid editing for algorithm reasons)
...
```
PLATFORM A → PLATFORM B STRATEGY:
├── Post in-depth content on your strongest platform
├── Screenshot best-performing post
├── Share on other platform as quote tweet / post with commentary
├── Screenshot format bypasses link penalty + creates visual scroll-stop
├── Followers discover your other platforms
...
2. Look for: same niche, active posters, genuine content
3. Start a DM group
...
├── Career threads — need genuinely unique story
...
├── X Premium = 2-4x visibility boost on all replies
├── Post originals TO COMMUNITIES (bypass cold-start)
...
KEY ACTIONS:
├── 1-2 original posts/day (Premium + communities for distribution)
├── Launch your first thread (career/origin story = pinned tweet)
├── Establish a core content pillar consistently
├── Cross-platform content bridge weekly
...
```
GOAL: Become "the person" for your content niche
RATIO: 30-40% original, 30-40% replies, 20% QTs

KEY ACTIONS:
├── Daily original post at consistent time
├── Weekly thread on your core topic
├── Launch newsletter — content flywheel
├── Guest on 1-2 podcasts/month
├── Submit best content to Hacker News
├── Host monthly X Space
├── Get quoted/referenced by bigger accounts
├── Start monetization prep (500 followers + 5M impressions/3mo)
└── Track: engagement rate per post, bookmark rate, reply quality

EXIT CRITERIA: 10K followers, recognized authority in niche
```
...
- One viral moment can add 500-2000 followers.
...
├── REVENUE TRANSPARENCY — monthly revenue screenshots
├── Simple vocab (8th grade), no periods, hedges takes
├── Mixes topics freely — not just tech
├── "Ship fast, talk about it" mentality

@SWYX (0 → 90K+) — Learn in Public:
├── Coined "learn in public" framework
...
├── Simple frameworks that feel novel
...
├── Long-form educational content → X amplification
...
| Metric | Target | How |
|--------|--------|-----|
| Follower growth | +10-20/week (Phase 1) | Profile page |
| Profile visits | Increasing week over week | Analytics |
| Reply engagement | Some replies getting 5+ likes | Check your replies |
| Content mix | No more than 40% in one domain | Audit your last 20 posts |
| Domain diversity | Posts/replies in 4+ domains | Audit your last 20 posts |
| Bookmarks | Track which content gets bookmarked | Analytics (with Premium) |

### What to Optimize

**If not growing:**
1. Are you posting during the money window? → Fix timing first
2. Are you only posting in one domain? → Diversify
3. Are your replies funny/interesting or just "insightful"? → Add humor
4. Are you engaging with trending non-niche posts? → Expand scope
5. Do people feel like they KNOW you? → Add personal content
6. Are you reacting to news fast enough? → Set up monitoring
7. Premium active? → Essential for reply visibility
...
## Available Actions (Browser Automation)
